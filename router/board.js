...
let upload = multer({
    ...
    limits: {
        fileSize: 1024 * 1024 * 5 // 5MB 까지 가능
    }
});
...
// 게시글 작성 (이미지 업로드 추가)
router.post('/v1/write', upload.single('image'), boardController.writeBoard);

// 게시글 수정 페이지 조회
router.get('/edit/:id', boardController.viewBoardEdit)

// 게시글 수정 (method-override 이용)
router.put('/v1/edit/:id', upload.single('image'), boardController.editBoard);
